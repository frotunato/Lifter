...
const gcloudPath = "/root/google-cloud-sdk/bin/gcloud";
...
(function init () {
	var initialWhitelist = require(whitelist);
	var currentOwner;
	for (var i = initialWhitelist.owners.length - 1; i >= 0; i--) {
		currentOwner = initialWhitelist.owners[i].name;
		if (!owners[currentOwner]) {
			owners[currentOwner] = {
				token: '',
				wallet: '',
				date: initialWhitelist.owners[i].date
			}
		}
	}
	for (ownerName in owners) {
			owners[ownerName].token = ('Bearer ' + execSync(`${gcloudPath} auth print-access-token ${ownerName}@gmail.com`)).replace('\n', '');
	}
...
var mainQ = async.queue(function (task, cb) {
	...
	} else if (task.action === 'addOwner') {
		updateWhitelist(task.owner, task.token, function (err) {
			log.alert('Added owner', task.owner, task.token, 'with error', JSON.stringify(err));
			cb(err);
		});
	}
}, 1);
...
function updateWhitelist (owner, token, callback) {
	console.log(owner, token);
	async.waterfall([
		function (wCb) {
			fs.readFile(whitelist, 'utf8', function (err, data) {
				var nData = JSON.parse(data);
				var index = depthIndexOf(nData.owners, 'name', owner);
				if (index !== -1) {
					err = (err) ? err : {err: 'Ya existe ese propietario'};
				}
				wCb(err, nData);
			});
	}, function (whitelistData, wCb) {
		const gcloud = spawn(gcloudPath, ['auth', 'login', owner + '@gmail.com']);
		var sentCredentials = false;

		function handleError (err) { 
			console.log(err);
			wCb(err, whitelistData);
		}
		process.once('uncaughtException', handleError);

		gcloud.on('error', function (err) {
		    //process.removeListener('uncaughtException', handleError);
		    wCb(err, whitelistData);
		});
		gcloud.on('close', function (code) {
			//console.log('THE CODE IS ' + code);
		    //process.removeListener('uncaughtException', handleError);
		    //wCb(null, whitelistData);
		    wCb((code == 0) ? null : {err: 'An error ocurred while an owner ' + owner + ' token: ' + token}, whitelistData);
		});
		gcloud.stderr.on('data', function (data) {
		    console.log(data + '');
		    if (!sentCredentials) {
		        sentCredentials = true;
		        gcloud.stdin.write(token + '\n');
		    }
		});
	}, function (whitelistData, wCb) {
		var ownerDate = Date.now();
		owners[owner] = {token: '', wallet: '', date: ownerDate};
		whitelistData.owners.push({name: owner, date: ownerDate});
		fs.writeFile(whitelist, JSON.stringify(whitelistData), function (err) {
			log.alert('[updateWhitelist] added', owner);
			wCb(err);
		});
	}], function (err) {
		callback(err);
	...
	async.waterfall([
		...
		function (status, wCb) {
						...
						function (cb) {
							setTimeout(function () {
								request({
									...
									var err = null;
									if (error || !body || !body.status || body.code == 403) {
										err = 'invalid response';
										//log.error('INSTANCE STARTING ERROR', body.message, 'DELETING...');
										//delete cache[obj.instance];
										//log.error('[controlMachine]', error, obj);
										currentStatus = 'TERMINATED';
									} else {
										currentStatus = body.status.replace('\n', '');
			...
			if (status === 'TERMINATED') {
				...
				request(options, function (error, response, body) {
					//log.alert('[controlMachine]', 'data testing: body', body);
...
function checkMachines (callback) {
	...
	for (instance in cache) {
		if (Date.now() - 600000 > cache[instance].timestamp) { //before cache[instance] ??
...
function renewTokens (callback) {
	...
	async.eachOf(owners, function (owner, ownerName, cb) {
		...
		var action = `${gcloudPath} auth print-access-token ${ownerName}@gmail.com`;
...
app.route('/owners/:owner/:token').post(function (req, res) {
	mainQ.push({action: 'addOwner', owner: req.params.owner, token: req.params.token.replace('$', '/')}, function (err) {
		console.log('Owner added', req.params.owner, req.params.token);
		res.json({err: err});
	});

	//updateWhitelist(req.params.owner, req.params.token, function (err) {
	//	res.json({err: err});
	//});
});
...
app.route('/check')
	...
	.post(function (req, res) {
		...
		req.body.owner = req.body.instance.substr(req.body.instance.lastIndexOf("-") + 1);
